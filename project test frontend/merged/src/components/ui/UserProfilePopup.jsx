...
const UserProfilePopup = ({ user, position, onClose }) => {
  ...
  return (
    ...
    >
      <style>{`.aura-popup{background:#1c1c1e;border:1px solid rgba(255,255,255,.1);border-radius:16px;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.7),0 0 0 1px rgba(255,255,255,.05)}`}</style>
      ...
      <div ref={popupRef} className="aura-popup" style={{ width: POPUP_W }}>
        ...
        <div className="px-3 pb-2 relative">
          ...
          <div className="relative -mt-7 mb-2 inline-block">
            ...
            <span
              className={`absolute bottom-0.5 right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#1c1c1e] ${statusCfg.bg}`}
            />
          </div>
            ...
            {!isOwnProfile ? (
              <>
                ...
                {friendStatus === "friends" ? (
                  <button disabled className="w-full py-2 rounded-xl bg-[#2c2c2e] text-gray-500 text-xs font-semibold border border-white/5 cursor-default">
                    ✅ Найзууд
                  </button>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-2 rounded-xl bg-[#2c2c2e] hover:bg-[#3a3a3c] border border-white/8 text-gray-300 hover:text-white text-xs font-semibold transition-all active:scale-[0.97] disabled:opacity-50"
                  >
                    👥 Найз болох
                  </button>
                )}
                ...
                <button
                  ...
                  className={`w-full py-2 rounded-xl text-xs font-semibold transition-all active:scale-[0.97] disabled:opacity-50 ${
                    isBlocked
                      ...
                      : "bg-[#2c2c2e] text-gray-500 hover:text-red-400 border border-white/8"
                  }`}
                >
              ...
              <button
                ...
                className="w-full py-2 rounded-xl bg-[#2c2c2e] hover:bg-[#3a3a3c] border border-white/8 text-gray-300 hover:text-white text-xs font-semibold transition-all active:scale-[0.97]"
              >
                ✏️ Профайл засах
              </button>
            )}
